...
import { LOAD_COMPONENTS_CONST } from "src/const";
...
const Loading: FC<MyLoadProps> = ({ load, className, ...props }) => {
  const infor_m: DotCircleLoadProps | LineCircleLoadProps | SkeletonLoadProps =
    load.infor;
    ...
    case LOAD_COMPONENTS_CONST.LOADING_TYPE.SKELETON: {
      ...
      if (
        !(
          ...
          (infor.maxminWidth === "max" ||
            infor.maxminWidth === "min" ||
            infor.maxminWidth === undefined) &&
          (typeof infor.height === "number" || infor.height === undefined) &&
          (infor.maxminHeight === "max" ||
            infor.maxminHeight === "min" ||
            infor.maxminHeight === undefined)
        )
      ) {
        console.warn({
          type: "Data type NOT valid",
  ...
  return (
    <div className={`TKS-Loading ${className || ""}`} {...props}>
