...
const Skeleton: FC<{ skeletonLoad: SkeletonLoadProps }> = ({
  skeletonLoad,
}) => {
    ...
    if (myElementRef.current) {
      if (skeletonLoad.maxminWidth === undefined) {
        ...
        if (skeletonLoad.maxminWidth === "max") {
          ...
          console.warn(
            "The maxminWidth value of skeletonLoad is invalid. It only recive values: [max, min]"
          );
        }
      }
  ...
  return (
    <div
      className="TKS-Load-Skeleton TKS-Load-Skeleton--Loading"
      ref={myElementRef}
    ></div>
  );
};
