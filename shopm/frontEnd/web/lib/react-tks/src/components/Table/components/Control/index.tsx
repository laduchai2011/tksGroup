...
const Control = () => {
  ...
  const [nextPageIndex, setNextPageIndex] = useState<number | undefined>(
    undefined
  );
  ...
  const handleLoad = useCallback((index: number): void => {
    if (q_loadingContainers.current[index]) {
      ...
      style_loadingContainer = getComputedStyle(
        q_loadingContainers.current[index]!
      );
  ...
  useEffect(() => {
    if (
      ...
      if (qq_selectPageContainer && nextPageIndex) {
        ...
        follow_loadingState.setData?.addState &&
          follow_loadingState.setData?.addState(LOAD_STATE.READY);
        q_pageIndexs[nextIndex.current].classList.add("selected");
      }
    }
  }, [
    nextPageIndexCluster,
    nextPageIndex,
    setPageIndex,
    follow_loadingState?.event,
    follow_loadingState?.setData,
  ]);

  return (
    <div className="TKS-Table-Control">
      <div
        className="TKS-Table-Control-selectPageContainer"
        ref={q_selectPageContainer}
      >
        <div>
          First
          <div ref={(el) => void (q_loadingContainers.current[0] = el)}>
            {loadDataState === LOAD_STATE.LOADING &&
              load !== undefined &&
              loadIndex === 0 && <Loading load={load} />}
          </div>
        </div>
        {amountOfPages() >= 1 && (
