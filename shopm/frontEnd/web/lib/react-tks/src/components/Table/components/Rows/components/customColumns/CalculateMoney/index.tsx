...
import { isNumberString, cluster_for_string, isFloat } from "src/utils/string";
...
const CalculateMoney: FC<{ rowIndex: number }> = ({ rowIndex }) => {
  ...
  const is_amount_input_negative: boolean = config.customColumn
    ?.is_amount_input_negative
    ? config.customColumn?.is_amount_input_negative
    : is_amount_input_negative_default;

  const amount_input_max: number | undefined =
    config.customColumn?.amount_input_max;
  if (amount_input_max) {
  ...
  const customColumn_values:
    | Table_Data_CustomColumn_DataIn_Type[][]
    | undefined = data.customColumn_values;

  const beforeInput = useRef<string>("0");
  useEffect(() => {
    if (
      dataTable !== undefined &&
      dataTable[rowIndex - 1] !== undefined &&
  ...
  const input_change = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (dataTable !== undefined && dataTable[rowIndex - 1] !== undefined) {
      ...
      event.customColumn?.onInputChange &&
        event.customColumn?.onInputChange(TKS);
      const value = e.target.value;
      setInput(value);
      setTitle(cluster_for_string({ string: value }));
      isNumberStringRef.current = isNumberString({
        string: input,
        isLog: true,
      });
    }
    ...
    if (dataTable !== undefined && dataTable[rowIndex - 1] !== undefined) {
      ...
      if (!isRemoveDefaultFunction) {
        if (isNumberStringRef.current) {
          ...
          if (!is_amount_input_negative) {
            // input can't is a negative number
            if (input_ >= 1) {
  ...
  const list_fields =
    ...
    fields.map((field: string, index: number) => {
      ...
      if (customColumn_values && customColumn_values[rowIndex - 1]) {
        const customColumn_values_thisRow: Table_Data_CustomColumn_DataIn_Type[] =
          customColumn_values[rowIndex - 1];
      ...
      return (
        <div key={index}>
          <div className="TKS-CalculateMoney--">
            <strong>-</strong>
          </div>
          <div
            className="TKS-CalculateMoney-text"
            title={moneyString_.clusted_string}
          >
            {moneyString_.full_with_round}
          </div>
        </div>
      );
    });
  ...
  return (
    <div className="TKS-CalculateMoney" ref={addSubElement}>
      <div>
        <SubCircle
          subCircle={{ size: 23, background: SUB_COLOR }}
        ...
        />
        <div className="TKS-CalculateMoney-fieldContainer">{list_fields}</div>
        <AddCircle
          addCircle={{ size: 23, background: ADD_COLOR }}
