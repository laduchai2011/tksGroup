...
const Rows: FC<{}> = () => {
    ...
    const element_rowsOfIndex: React.RefObject<(HTMLDivElement | null)[]> = elements.current.rowsOfIndex;
    const element_rows: React.RefObject<(HTMLDivElement | null)[]> = elements.current.rows;
    const element_rowsOfCalculate: React.RefObject<(HTMLDivElement | null)[]> = elements.current.rowsOfCalculate;
    ...
    useEffect(() => {
        if (elementRows.current) {
            ...
            customColumn_maxWidth &&
                elementRows.current.style.setProperty('--customColumn-maxWidth', customColumn_maxWidth);
        }
    ...
    if (config?.columnsInfor && rowHeader?.cells && rowForm?.cells) {
        for (let i: number = 0; i < config.columnsInfor.length; i++) {
            rowHeader.cells.push(
                cellHeader(config.columnsInfor[i].fieldName, config.columnsInfor[i].columnName, 'black', '700')
            );
        ...
        totalRow.current.map((data: RowProps, index: number) => {
            if (index === 0) {
                return (
                    ...
                    >
                        {isCustomColumn && (
                            <BigRightArrow className="TKS-Rows-right-header-svg1" onClick={() => handleClickRight()} />
                        )}
                        {!isCustomColumn && (
                            <BigLeftArrow className="TKS-Rows-right-header-svg2" onClick={() => handleClickLeft()} />
                        )}
            ...
            return (
                <div
                    ...
                    onMouseDown={(e) => handleMouseDown_row(e)}
                >
                    <div>{loadDataState === LOAD_STATE.LOADING && <Loading load={load} />}</div>
