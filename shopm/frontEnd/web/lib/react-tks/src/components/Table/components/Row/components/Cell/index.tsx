...
const Cell: FC<{
  ...
  useEffect(() => {
    ...
    if (q_Cell) {
      ...
      data?.textColor &&
        q_Cell.style.setProperty("--Cell-textColor", data?.textColor);
      data?.textWeight &&
        q_Cell.style.setProperty("--Cell-textWeight", data?.textWeight);
    }
  ...
  return (
    <div
      className="TKS-Cell"
      ref={(el) => void (element_cells.current[cellIndex] = el)}
    >
      <div>
        {loadDataState === LOAD_STATE.LOADING && rowIndex !== 0 && (
          <Loading load={load} />
        )}
      </div>
      ...
      <div
        onMouseDown={(e) =>
          handleMouseDown(e, TABLE_CONST.RESIZABLE.CELL.TYPE.WIDTH)
        }
      ></div>
      <div
        onMouseDown={(e) =>
          handleMouseDown(e, TABLE_CONST.RESIZABLE.CELL.TYPE.HEIGHT)
        }
      ></div>
    </div>
  );
