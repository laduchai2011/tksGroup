...
const ToastMessage: FC<MyToastMessageProps> = ({
  toastMessage,
  className,
  ...props
}) => {
  ...
  useEffect(() => {
    ...
    if (toastMessage?.data) {
      ...
      if (toastMessageContainerElement.current) {
        const messageBoxDivElements_m: HTMLCollectionOf<HTMLDivElement> =
          toastMessageContainerElement.current
        ...
        for (let i: number = 0; i < messageBoxDivElements.length; i++) {
          const messageBoxElements: HTMLCollectionOf<HTMLDivElement> =
            messageBoxDivElements[i]
              .children as HTMLCollectionOf<HTMLDivElement>;
          if (messageBoxElements[0]) {
            ...
            messageBoxElements[0].style.top = `${
              old_top + distanceMessageBoxs
            }px`;

            if (i === maxMessage_.current) {
              messageBoxElements[0].style.opacity = "0";

              const messageBoxDivElements_n: HTMLDivElement =
                messageBoxDivElements[i];
  ...
  const createElement = (createElement: CreateElementProps): void => {
    ...
    toastMessageContainerElement.current?.insertBefore(
      newNode,
      toastMessageContainerElement.current.firstChild
    );
  };
  ...
  return (
    ...
    >
      <div
        className="TKS-ToastMessage-Container"
        ref={toastMessageContainerElement}
      ></div>
    </div>
  );
};
