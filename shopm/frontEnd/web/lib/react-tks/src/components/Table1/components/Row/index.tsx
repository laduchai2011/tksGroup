...
const Row: FC<{ table1_CCRow?: Table1_CCRow_Props }> = ({ table1_CCRow }) => {
  ...
  useEffect(() => {
    ...
    for (let i: number = 0; i < columnAmount; i++) {
      list_index_of_cell_thisRow.push(
        handleTableIndex(columnAmount, rowIndex, i)
      );
    }
    let mostheight: number = 0;
    for (let i: number = 0; i < cell_element.current.length; i++) {
      if (list_index_of_cell_thisRow.indexOf(i) !== -1) {
        const height_: string | undefined = window.getComputedStyle(
          cell_element.current[i]!
        ).height;
    ...
    for (let i: number = 0; i < cell_element.current.length; i++) {
      if (list_index_of_cell_indexColumn.indexOf(i) !== -1) {
        const width_: string | undefined = window.getComputedStyle(
          cell_element.current[i]!
        ).width;
  ...
  const list_cell: React.ReactNode = table1_CCRow?.data?.cells.map(
    (data: Table1_CCCell_Props, index: number) => {
      cellIndex = index;
  ...
  return <div className="TKS-Table1-Row">{list_cell}</div>;
};
