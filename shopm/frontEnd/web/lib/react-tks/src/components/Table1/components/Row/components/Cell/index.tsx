...
const Cell: FC<{ table1_CCCell?: Table1_CCCell_Props }> = ({
  table1_CCCell,
}) => {
  ...
  const config_cell: Table1_CCCell_Config_Props | undefined =
    table1_CCCell?.config;
  const cell_element: React.MutableRefObject<(HTMLDivElement | null)[]> =
    elements.current.cell_element;

  const index_in_table: number | undefined =
    table1_CCCell?.config?.index_in_table;
  ...
  useEffect(() => {
    if (index_in_table !== undefined && cell_element.current[index_in_table]) {
      config?.cell_maxWidth &&
        cell_element.current[index_in_table]!.style.setProperty(
          "--maxWidth",
          config.cell_maxWidth
        );
