...
const Cell: FC<{ table1_CCCell?: Table1_CCCell_Props }> = ({ table1_CCCell }) => {
    ...
    useEffect(() => {
        if (index_in_table !== undefined && cell_element.current[index_in_table]) {
            config?.cell_maxWidth &&
                cell_element.current[index_in_table]!.style.setProperty('--maxWidth', config.cell_maxWidth);

            config_cell?.text_color &&
                cell_element.current[index_in_table]!.style.setProperty('--text-color', config_cell.text_color);
            config_cell?.text_weight &&
                cell_element.current[index_in_table]!.style.setProperty('--text-weight', config_cell.text_weight);
            config_cell?.width && cell_element.current[index_in_table]!.style.setProperty('--width', config_cell.width);
            config_cell?.height &&
                cell_element.current[index_in_table]!.style.setProperty('--height', config_cell.height);
        }
    ...
    if (index_in_table !== undefined) {
        return (
            <div
                ...
                ref={(el) => {
                    if (index_in_table !== undefined) {
                        cell_element.current[index_in_table] = el;
                    }
                }}
            >
                <div>{content}</div>
