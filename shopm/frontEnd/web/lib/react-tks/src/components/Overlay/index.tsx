...
const Overlay: FC<MyOverlayProps> = ({ overlay, className, ...props }) => {
  ...
  useEffect(() => {
    if (overlayElement.current) {
      overlay?.config?.zIndex &&
        overlayElement.current.style.setProperty(
          "--zIndex",
          `${overlay.config.zIndex}`
        );
      overlay?.config?.opacity_time &&
        overlayElement.current.style.setProperty(
          "--opacity-time",
          `${overlay.config.opacity_time}`
        );
      overlay?.config?.show_time &&
        overlayElement.current.style.setProperty(
          "--show-time",
          `${overlay.config.show_time}`
        );
      overlay?.config?.blear_rate &&
        overlayElement.current.style.setProperty(
          "--blear-rate",
          `${overlay.config.blear_rate}`
        );
  ...
  useEffect(() => {
    if (overlayElement.current) {
      if (
        ...
        overlay?.control?.isCenter === undefined
      ) {
