...
const Center = () => {
    ...
    const [is_CommentInput, set_is_CommentInput] = useState<boolean>(false);
    const [is_text_hidden, set_is_text_hidden] = useState<boolean>(true);
    ...
    const video_src = 'http://192.168.5.100:3007/api/service_video/get/watch?id=video.mp4';
    // const video_src = 'http://192.168.5.100:3007/api/service_video/get/watch?name=1280_720_p_003.ts';

    const handle_reply = () => {
        set_is_CommentInput(!is_CommentInput);
    };

    const handle_className_text_hidden = () => {
        if (is_text_hidden) {
            return style.text_hidden;
        }
        return '';
    };

    const handle_text_hidden = () => {
        set_is_text_hidden(!is_text_hidden);
    };
    ...
    return (
        <div className={style.parent} ref={parent_element}>
            ...
            <div>
                <div>
                    <div className={handle_className_text_hidden()}>{ctn}</div>
                    <div onClick={() => handle_text_hidden()}>
                        {is_text_hidden && <div>{SEE_MORE}</div>}
                        {!is_text_hidden && <div>{HIDDEN}</div>}
                    </div>
                </div>
                <div>
                    {/* <LazyImage src={img} alt="img" /> */}
                    <div>
                        <ImageOverview src={img} />
                    </div>
                    <div>
                        <VideoOverview src_img={img} src_video={video_src} />
                    </div>
                </div>
                <div>
                    <div>
                ...
                <div>{is_CommentInput && <CommentInput />}</div>
            </div>
        </div>
    );
};
