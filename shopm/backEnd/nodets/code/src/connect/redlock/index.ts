...
// Base Redis URL
const baseURL_shopm: string = `redis://${redisConfig()?.username}:${redisConfig()?.password}@${redisConfig()?.host}:${
    redisConfig()?.port
}`;
...
redisLockClient.on('connect', () => {
    my_log.withGreen('Redis Lock client connected!');
});

redisLockClient.on('reconnecting', () => {
    my_log.withCyan('Redis Lock client reconnecting...');
});
